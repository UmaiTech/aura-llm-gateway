...
1. **Frontend-only commits get dropped.** Anything that touches only `apps/admin/`, `apps/landing/`, `apps/chat/`, `sdks/`, `.github/`, `AGENTS.md`, or any other non-`crates/` path is **not attributed to any crate** and never reaches the auto-generated entry. We hit this for v0.11.0–v0.13.0 where most work was frontend.
