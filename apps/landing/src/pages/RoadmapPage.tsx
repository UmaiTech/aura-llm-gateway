...
const releases: Release[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'TypeScript SDK', note: 'aura-llm on npm — universal fetch, AsyncIterable streaming, typed errors; matches the Python feature set.' },
      { label: 'Fireworks AI provider', note: 'OpenAI-compatible adapter for Fireworks’ serverless open-weight catalog (GLM, Kimi, DeepSeek, Qwen, gpt-oss) — a second day-zero routing target alongside Together. Pricing auto-refreshes from the scraped price history.' },
      ...
      { label: 'Streaming fixes for Gemini and long answers', note: 'Gemini stream chunks were treated as cumulative and lost the start of every longer chunk; network chunks cut inside a multi-byte character were dropped by every provider. Both fixed, the playground throttles per-token re-renders and skips syntax highlighting until a code block finishes, and every decision now echoes the routing options it applied.' },
